...
class RAGSystem:
    ...
    def process_query(self, question):
        try:
            ...
            enhanced_query = self.query_enhancing(
                question)  # query_enhancing 함수 호출
            logger.info(f"Enhanced query: {enhanced_query}")
            ...
            # Log first 100 characters of the answer
    ...
    def ingest_and_process(self, file_path):
        try:
            ...
            self.db = create_vectorstore(
                split_docs, self.embeddings, CHROMADB_PATH)
            self.retriever = create_retriever(
                self.db, search_type="similarity", k=7)
            self.chain = create_chain(self.retriever, self.prompt, self.llm)
